<footer>
  <div class="smicons">
    <a href="https://facebook.com/bigdatasgh/" target="_blank"><img src="<?php echo get_template_directory_uri(); ?>/img/fb-icon.png"></a>
    <a href="https://www.linkedin.com/company-beta/18024974/" target="_blank"><img src="<?php echo get_template_directory_uri(); ?>/img/in-icon.png"></a>
    <a href="https://www.youtube.com/channel/UC21Ea5Z86aDbKIoL08Ngn9Q" target="_blank"><img src="<?php echo get_template_directory_uri(); ?>/img/yt-icon.png"></a>
  </div>
  <p>Copyright © 2017 SKN Data Science Management</p>
</footer>
<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.2.0/jquery.min.js"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/jquery-cookie/1.4.1/jquery.cookie.min.js"></script>
<script src="<?php echo get_template_directory_uri(); ?>/js/main.js"></script>
<script src="<?php echo get_template_directory_uri(); ?>/js/bootstrap.min.js"></script>
<link rel="stylesheet" href="<?php echo get_template_directory_uri(); ?>/style/owl.carousel.min.css">

<script src="<?php echo get_template_directory_uri(); ?>/js/owl.carousel.js"></script>
<script src="<?php echo get_template_directory_uri(); ?>/js/parsley.js"></script>
<div id="popup" class="modal fade" role="dialog">
<div class="modal-dialog modal-md event-description">

  <!-- Modal content-->
  <div class="modal-content modalAnimation3">
    <div class="modal-header">
      <button type="button" class="close" data-dismiss="modal">&times;</button>
      <h2 class="modal-title" style="text-align:center;color:#fff;">Rekrutacja trwa, dołącz do Nas już dziś!</h2>
    </div>
    <div class="modal-body">
      <a href="rekrutacja/"><div class="button">Przejdź do formularza</div></a>
    </div>

  </div>

</div>
</div>

<div class="cookie-policy col-sm-6 " style="display: none;" id="cookiePolicy">
  <div class="col-xs-2"><svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="50px" height="50px"
  	 viewBox="0 0 55 55" style="enable-background:new 0 0 55 55; margin-top: 5px;" xml:space="preserve">
  <g>
  	<path style="fill:#A46F3E;" d="M42.48,25.681l4.669-2.146c2.919-0.793,4.659-3.83,3.867-6.749c-0.793-2.919-3.83-4.659-6.749-3.866
  		l-7.312,5.053C36.304,18.272,35.603,17.7,35.754,17c0.177-0.82,1.962-0.969,1.962-5.068C37.716,7.289,36.764,0,28.456,0
  		c-8.723,0-9.723,6.679-9.723,12c0,3.063,1.848,4.369,1.981,5c0.144,0.685-0.544,1.243-1.186,0.965l-8.794-5.581
  		c-2.919-0.793-5.956,0.947-6.749,3.867h0C3.191,19.17,4.931,22.207,7.85,23l6.007,2.601c2.011,1.232,5.846,4.412,4.022,9.966
  		l-4.921,8.59c-2.728,0.655-4.685,3.351-4.131,6.37C9.312,53.162,11.739,55,14.42,55h1.813c0.079,0,0.156-0.009,0.235-0.012
  		c0.085-0.004,0.17-0.005,0.254-0.013c0.283-0.025,0.558-0.075,0.827-0.142c0.069-0.017,0.138-0.034,0.206-0.053
  		c0.267-0.078,0.524-0.176,0.773-0.291c0.079-0.036,0.156-0.073,0.233-0.113c0.227-0.118,0.443-0.253,0.65-0.4
  		c0.09-0.064,0.178-0.128,0.264-0.198c0.19-0.153,0.368-0.321,0.536-0.498c0.077-0.081,0.153-0.161,0.225-0.248
  		c0.175-0.208,0.332-0.432,0.476-0.664c0.027-0.044,0.063-0.078,0.088-0.123l5.573-7.895c0.809-1.146,2.509-1.146,3.318,0
  		l5.573,7.895c0.026,0.045,0.061,0.079,0.088,0.123c0.144,0.233,0.3,0.456,0.476,0.664c0.072,0.086,0.148,0.167,0.225,0.248
  		c0.168,0.177,0.346,0.344,0.536,0.498c0.086,0.069,0.174,0.134,0.264,0.198c0.207,0.148,0.423,0.282,0.65,0.4
  		c0.077,0.04,0.154,0.076,0.233,0.113c0.249,0.115,0.506,0.214,0.773,0.291c0.069,0.02,0.137,0.036,0.206,0.053
  		c0.269,0.067,0.544,0.117,0.827,0.142c0.085,0.008,0.169,0.009,0.254,0.013C40.077,54.991,40.154,55,40.233,55h1.813
  		c2.68,0,5.108-1.838,5.592-4.474c0.555-3.019-1.402-5.715-4.131-6.37l-4.921-8.59C36.801,30.129,40.439,26.967,42.48,25.681z"/>
  	<circle style="fill:#ED3F32;" cx="28.233" cy="35.5" r="2.5"/>
  	<circle style="fill:#88C057;" cx="28.233" cy="26.5" r="2.5"/>
  	<circle style="fill:#FFF5BB;" cx="31.733" cy="7" r="2"/>
  	<circle style="fill:#FFF5BB;" cx="24.733" cy="7" r="2"/>
  	<path style="fill:#FFF5BB;" d="M28.233,16.375c-2.661-0.027-5.5-1.792-5.5-4.375c0-0.553,0.447-1,1-1s1,0.447,1,1
  		c0,1.202,1.706,2.356,3.511,2.375c1.756,0,3.489-1.177,3.489-2.375c0-0.553,0.447-1,1-1s1,0.447,1,1
  		C33.733,14.578,30.835,16.375,28.233,16.375z"/>
  	<path style="fill:#FFF5BB;" d="M43.937,13.148l-1.659,1.146l4.818,9.266l0.053-0.024c0.649-0.176,1.235-0.468,1.752-0.84
  		L43.937,13.148z"/>
  	<polygon style="fill:#FFF5BB;" points="32.435,47.953 42.344,42.125 41.35,40.389 31.278,46.313 	"/>
  	<path style="fill:#FFF5BB;" d="M34.43,50.78l10.395-6.115c-0.412-0.222-0.852-0.397-1.317-0.509l-0.444-0.776l-9.791,5.759
  		L34.43,50.78z"/>
  	<polygon style="fill:#FFF5BB;" points="25.686,45.608 15.145,40.338 14.15,42.076 24.519,47.261 	"/>
  	<path style="fill:#FFF5BB;" d="M22.643,49.919l1.17-1.657l-10.352-4.984l-0.503,0.879c-0.449,0.108-0.874,0.276-1.273,0.486
  		L22.643,49.919z"/>
  	<path style="fill:#FFF5BB;" d="M6.733,22.556C7.083,22.74,7.454,22.893,7.85,23l0.733,0.317l3.923-9.808l-1.718-1.09L6.733,22.556z
  		"/>
  </g>
  <g>
  </g>
  <g>
  </g>
  <g>
  </g>
  <g>
  </g>
  <g>
  </g>
  <g>
  </g>
  <g>
  </g>
  <g>
  </g>
  <g>
  </g>
  <g>
  </g>
  <g>
  </g>
  <g>
  </g>
  <g>
  </g>
  <g>
  </g>
  <g>
  </g>
</svg></div>
  <div class="col-xs-10"><p>Nasza strona wykorzystuje ciasteczka aby dostarczać treści najwyższej jakości. Jeżeli akceptujesz kliknij przycisk. Więcej informacji dostępnych w regulaminie.</p></div>
  <div class="btn accept" id="accept">Akceptuję</div>
  <a href="/cookies.pdf" target="_blank"><div class="btn decline">Zobacz więcej</div></a>
</div>
<script>
$(document).ready(function(){
$(".owl-carousel").owlCarousel({
  nav: true,
  margin: 25,
  center: false,
  loop: false,
  responsive: {
    1180:{
      items: 3,
    },
    680:{
      items: 2,
    },

    0:{
      items: 1,
    }
  }

});
});

$(document).ready(function(){
  //REKRUTACJA POPUP
// if ($.cookie('popup') == null) {
// setTimeout(function(){
//     $('#popup').modal('toggle');
// }, 5000);
// $.cookie('popup', '7', {expires: 30, path: '/'});
// }
if ($.cookie('cookiePolicy') == null) {
  setTimeout(function(){
      $('#cookiePolicy').fadeIn();
  }, 500);
  $('#accept').click(function(){
    $.cookie('cookiePolicy', '7', {expires: 15000, path: '/'});
    setTimeout(function(){
        $('#cookiePolicy').fadeOut();
    }, 500);
  })
}

});



function nextModal(id) {
 console.log(id);
  $(".modal").modal('hide');
  $("#" + id).modal();
}
</script>

<?php
if ( is_front_page() ) { ?>
<script>
  $("#submit").click(function(){
    var email = $("#email").val();
    var name = $("#name").val();
    var agree = $('#agree').prop('checked');
    var thankYou =  "<h3>Dziękujemy za przesłanie zgłoszenia.</h3></br><p>Dodatkowe informacje o dostępnych projektach zostana wysłane na podany adres email.</p>";
    if((email == '') || (name == '') || (agree == false)) {
      $("#message").html("<div class=\"alert alert-danger alert-dismissable\"><button type=\"button\" class=\"close\" data-dismiss=\"alert\" aria-hidden=\"true\">&times;</button>Uzupełnij wszystkie pola</div>");
    }
    else {
      $.ajax({
        type: "POST",
        url: "<?php echo get_template_directory_uri(); ?>/sendProject.php",
        data: "name="+name+"&email="+email,
        success: function(html){

      var text = $(html).text();
      //Pulls hidden div that includes "true" in the success response
      var response = text.substr(text.length - 4);

          if(response == "true"){

      $("#message").html(html);
      setTimeout(function(){
        $('.projectForm').text('');
        $('#thankyou').html(thankYou);

      },1000);
          setTimeout(function(){
            $('#signforproject').modal('hide');

          },8000);
      }
    else {
      $("#message").html(html);

      }
        },
        beforeSend: function()
        {
          $("#message").html("<p class='text-center'><img src='<?php echo get_template_directory_uri(); ?>/img/ajax-loader.gif'></p>")
        }
      });
    }
    return false;
  });
  </script>

<?php }; ?>


</body>
</html>
